...
const createScene = function () {
  ...
  const otherPoint = (obj, parent) => ({
    x: obj.x, y: obj.y, parentX: parent.x, parentY: parent.y,
  });
  let processingStack = [secondObj.path];
  let arrayOfSnags = [];
  let snag = [];

  const createSnag = (startPoint) => [startPoint];
  ...
  const scale = 20;
  const snagToVectorArray = (snag) => snag.map(([x, y]) => new Vector3(x * scale, y * scale, 0));
